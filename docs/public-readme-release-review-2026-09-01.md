...
- 2026-09-01 최신 로컬 개발 빌드 캡처로 사옥·분석 보관함·통합 모의투자·append-only 원장 이미지를 교체하고 연결·계좌 설정 화면을 추가한다.
- 설정 캡처는 연결 완료·추가 확인 필요·미연결 상태를 함께 보여주되 API Key·Secret, 전체 계좌번호와 공급자 사용자 ID는 포함하지 않는다.

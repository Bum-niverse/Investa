...
| 국내 공시·재무 | 금융감독원 OpenDART | 선정·어댑터 대기 | 인증키 발급 후 공시·재무 PIT 어댑터 |
| 국내 일반 뉴스 | 네이버 뉴스 검색 API | 선정·어댑터 대기 | 앱 등록·키 저장·중복/출처 정규화 |
...
현재 구현은 공통 설정·상태·단일 분석 REST 계약까지다. 기존 44인 직원 오케스트레이션은 여전히 Codex App Server를 사용하며, 구조화 `RoleReport` 검증과 스트리밍·취소·사용량 계약을 공통화한 뒤에만 다른 공급자로 전환할 수 있다.

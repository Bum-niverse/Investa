...
위 GIF는 로그인 화면이 아니라 실제 로컬 개발 빌드의 `사옥 → 분석 기록 → 통합 모의투자 → append-only 원장`을 순서대로 보여줍니다. 아래 잔고·성과·주문은 내부 모의 데이터와 시스템 연결 검사이며 실제 계좌나 실주문 결과가 아닙니다.

| 픽셀 투자본부 사옥 | 분석·백테스트 기록 |
| --- | --- |
| ![9개 조직과 44명의 역할을 배치한 Investa 픽셀 투자본부](docs/assets/investa-office.jpg) | ![요청·완료 시각과 검증 결과를 보존하는 Investa 분석 보관함](docs/assets/investa-analysis.jpg) |
| **차트·통합 모의투자** | **append-only 모의 원장** |
| ![캔들 차트와 보조지표, 실주문 잠금이 표시된 통합 모의투자 화면](docs/assets/investa-paper-trading.jpg) | ![KRW와 USD를 분리해 내부 체결 사건을 표시하는 모의 원장](docs/assets/investa-ledger.jpg) |

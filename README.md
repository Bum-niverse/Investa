...
전체 보유종목 분석은 연결된 토스증권 계좌의 구조화 종목코드·수량·현재가·평단가와 KRW/USD 현금 기반 매수 가능 금액을 읽기 전용으로 가져옵니다. 계좌 식별정보는 AI 근거에서 제거하며, 다종목 결과는 종목별 PIT 시각·통화·부분 실패를 유지하고 실주문으로 이어지지 않습니다.
...
- **장문 종합:** 직원·부서 원문은 로컬 기록에 보존하고 본부장에게는 부서별 중간 결과와 중복 제거된 근거를 단계적으로 전달합니다. 최종 종합이 실패해도 완료 부서 보고와 실패 이유가 `hold` 기록으로 남습니다.
...
`내 보유종목 전체 분석`처럼 요청하면 토스증권의 읽기 전용 보유내역을 종목별 PIT 스냅샷으로 조립해 한 회의에서 함께 검토한다. 계좌 식별정보는 제거하고 종목·통화·수량·현재가·평단가와 근거 ID만 전달하며, 부분 실패와 혼합 통화를 숨기지 않는다. 설정의 포트폴리오 운용 원칙은 관측 전용·집중·테마·분산·사용자 정의로 구분한다. 집중 한도를 사용자가 명시적으로 활성화하지 않으면 기술주·테마주 비중을 앱이 임의로 위반·감축·매도 사유로 판단하지 않는다. 다종목 보고는 사용자가 한 종목을 다시 선택하기 전 백테스트나 모의주문 후보로 직접 인계하지 않는다.

분석 보관함의 `시장 후보 탐색`은 토스증권 국장·미장 시장 랭킹을 1차 필터로 사용한다. 시장 전체 거래대금·거래량과 급등락 상위 목록을 합친 뒤 상위 12종목에만 완료 일봉을 요청해 균형·추세·반전 관찰 규칙을 결정론적으로 검증한다. 최대 5개 후보는 근거와 제외·부분 실패 수를 함께 표시하고, 선택하면 전체 분석 회의 안건만 작성한다. 후보 탐색은 추천·백테스트·주문 승인이 아니며 호가 스프레드와 장 상태는 모의주문 전에 다시 검증한다.
...
퀀트 논문 연구원 요청에 공개 GitHub 저장소 URL이 있으면 최대 2개 저장소의 공개 메타데이터, HEAD commit, 라이선스 후보와 README 일부를 GitHub API에서 읽기 전용으로 수집합니다. 논문·학술 연구 요청이면 Crossref에서 서지 메타데이터 후보를 최대 5건 수집합니다. 리서치부 직원은 최종 보고 전에 `AgentToolPlan`으로 역할별 허용 도구를 최대 3개 선택하며, Rust와 프런트가 계획과 결과 전달을 각각 검증한 뒤 선택한 결과만 `RoleReport`에 제공합니다. 서지 메타데이터는 원문 검증·전략 성과·재현 성공을 뜻하지 않으며, 임의 도메인·private 저장소·코드 다운로드와 실행은 지원하지 않습니다. 외부 내용은 명령이 아닌 신뢰할 수 없는 자료로 취급합니다.

Telegram 신규 동기화에는 인터넷 연결이 필요합니다. 연결이 끊기면 Investa가 이전에 선택 채널에서 SQLite로 동기화한 메시지만 `cached_offline` 상태와 마지막 관측 시각을 붙여 분석 근거 후보로 사용합니다. Telegram 앱 자체의 로컬 캐시는 읽지 않으며 저장 메시지를 최신 뉴스로 표시하지 않습니다.
...
- 부서 회의 상태는 추정 백분율이나 가짜 타이머가 아니라 Codex turn 시작, 첫 구조화 응답 조각 수신, 계약 검증 시작과 검증 완료 이벤트에만 따라 바뀝니다. 본부를 제외한 8개 전문 부서의 직원은 각각 역할별 도구 계획과 독립 `RoleReport`를 만들고 부장이 실제 결과만 취합합니다. 중요 회의 호출 예산은 전체 35명까지 실행 가능한 최대 80회이며, 실제 라우팅된 전문 부서는 모두 직원별로 실행합니다. 부서장이 실행되지 않은 직원의 소견을 대신 작성하지 않습니다.
...
- 안건은 `일반`(최대 5회 호출)과 `중요`(최대 80회 호출)로 구분하고 동시 실행은 2명으로 제한합니다. 전문 부서가 필요한 안건은 중요로 자동 승격하며 자동 분류 호출도 예산에 포함됩니다. Codex 사용량을 확인할 수 없거나 80% 이상이면 새 안건을 시작하지 않으며, 분류 후에도 사용량을 다시 검사합니다. 분류 계약 실패·알 수 없는 부서 ID·보고 실패는 안전하게 회의를 중단하거나 기각 보고로 바꾸고, 본부장 종합 실패는 항상 `보류`로 닫습니다.
...
회의와 구조화 연구 업무는 추정 백분율 대신 업무 대기, 요청 전달, 응답 생성, 계약 검증과 보고 완료 상태를 표시합니다. 리서치부 회의 직원 상태는 실제 독립 turn을 뜻하고, 다른 부서의 직원 상태는 아직 부서장 단일 구조화 응답의 역할 배열을 시각화한 것입니다. 개별 직원에게 직접 맡긴 업무는 별도의 역할 한정 계약으로 검증되며, 결과는 분석 보관함에 `개별 역할 소견`으로 저장됩니다. 부장에게 직접 지시한 경우에는 사용자가 배정안을 승인해야 직속 직원 실행과 부서 종합이 시작됩니다.
...
왼쪽 `원장`의 첫 화면은 토스증권 읽기 전용 계좌에서 조회한 실제 보유자산을 통화별 도넛 차트·비중·평가손익·상세 표로 보여줍니다. 분석 기록에는 분석 당시의 익명화된 포트폴리오 스냅샷, 종목별 완료 OHLCV·이동평균·거래량·지지·저항 주석 차트와 Telegram 근거 수집 상태가 함께 저장됩니다. 부서 보고의 백분율은 방향 예측이 아닌 근거 품질 자체평가로 표시하며 고유 근거 ID와 근거 공백을 함께 확인합니다. 예상 배당과 업종 비중은 공식 분류·배당 데이터 공급자가 연결되기 전까지 추정하지 않으며, 계좌 식별자와 금융 자격정보는 기록에 포함하지 않습니다.
...
퀀트 논문 연구원은 사용자가 직접 연구를 지시하거나 회의에서 공개 웹 조사를 선택한 경우에만 별도 격리 Codex thread로 공식 문서·원 논문·upstream을 검색합니다. 일반 직원의 웹 검색과 shell network는 꺼져 있으며 계좌·보유수량·현금·개인정보는 검색어로 전달하지 않습니다. 별도 유료 검색 API는 사용하지 않고 로그인된 Codex 구독 사용량만 적용합니다.

...
한국·미국 주식과 암호화폐를 여러 전문 에이전트가 분석하고, 결정론적 위험 게이트를 통과한 거래안만 모의투자로 검증하는 로컬 우선 Windows 데스크톱 프로그램입니다.
...
- 기본 로그인은 개발자·로컬 배포 성격에 맞춰 GitHub 하나만 실제 지원합니다. 설치된 GitHub CLI의 `github.com` 세션을 확인하고 공개 프로필의 ID·로그인명만 앱 진입 식별에 사용합니다.
- Investa는 GitHub access token을 읽어 화면·SQLite·로그에 저장하지 않습니다. 금융 API 키와 GitHub 로그인은 별도 경계입니다.
- 이 기능은 로컬 앱 진입을 구분하는 게이트이며 SQLite 파일 암호화나 Windows 계정 잠금을 대신하지 않습니다.
- Google·Apple 등 일반 소비자용 소셜 로그인은 원격 세션 서버, 계정 복구, 개인정보처리방침과 배포 운영 모델을 확정한 뒤 추가합니다. 현재 화면의 항목은 비활성 `검토 중` 표시이며 로그인 기능으로 가장하지 않습니다.
...
회의는 라우팅·소집·부서 분석·재소집·결과 단계와 부서 보고를 SQLite 체크포인트로 저장합니다. 앱이 비정상 종료되면 미완료 회의를 `중단됨`으로 표시하고, 손실된 Codex 실행을 임의로 이어 붙이지 않고 같은 안건을 처음부터 재실행하거나 기록을 닫도록 합니다. 한국투자증권 계좌 개설과 외부 모의계좌 검증은 사용자 결정에 따라 보류했으며, 준비 항목은 [KIS 모의투자 연결 준비](docs/kis-paper-trading-connection.md)에 정리했습니다.
...
모든 신규 기능은 [개발 레퍼런스 적용 규칙](docs/development-reference-policy.md)에 따라 기존 코드와 공식 문서·원 논문·공개 저장소를 먼저 확인합니다. 외부 코드는 곧바로 실행하지 않고 라이선스·데이터 가정·시점 누수와 보안 경계를 검토한 뒤 필요한 로직만 재구현합니다.
